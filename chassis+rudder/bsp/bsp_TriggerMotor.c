...
void TriggerMotor_control(void)
{
    ...
    switch( shoot.stir_mode )
    {
        ...
        case STIR_MODE_STOP:     //拨盘停止模式，保持静止，有力
        {
            ...
            if( STIR_MODE_PROTECT == shoot.stir_mode )
            {
            ...
            else if( STIR_MODE_SERIES == shoot.stir_mode )
            {
              /* if( ( rc.mouse.l || ctrl_mode == REMOTER_MODE ) && \
                    ...
                    && ABS(shoot.barrel.pid.trigger_ecd_error) < 0.8f * TRIGGER_MOTOR_ECD \
                     ...
                     if( ( vision_ctrl.shoot_cmd|| ctrl_mode == REMOTER_MODE ) && \
                    frequency_cnt * SHOOT_PERIOD >= shoot.trigger_period \
                    && ABS(shoot.barrel.pid.trigger_ecd_error) < 0.8f * TRIGGER_MOTOR_ECD \
                     )//一个周期打一颗  射频控制  && shoot.barrel.heat_remain >= MIN_HEAT
                {
                  //  uint8_t shoot_enable_flag = 0;
                    if( FLAG_VISION_ANTIROTATE == KEY_RUN )  //返校陀螺，自动开枪条件
                    {
                        /*if( ABS(vision.yaw.angle_error[NOW] < 2 && \
                            ABS(vision.pit.aim_speed[NOW]) < 80 && \
                            ABS(imu_data.wy) < 80) )
                        {
                            shoot_enable_flag = 1;
                        }
