...
void vsn_calc(void)
{
    ...
    vd.pos.now = imu_data.yaw + vision_ctrl.yaw; /* 子弹击打角 */
    ...
    /* 视觉预测流程 */
    if (vd.valid_cnt && vsn_drive.status != AIMING)
    { /* 首次丢帧或异常重力下坠无解 */
        vd.valid_cnt = 0;
        vsn_deinit();                /* 视觉历史数据清空 */
        vision.status = vFIRST_LOST; /* 输出为首次丢帧状态 */
    }
    else if (vsn_drive.status == AIMING)
    {                              /* 识别到目标 */
        ++vd.valid_cnt;            /* 有效数据计数 */
...
static void vsn_kal_reinit(void)
{ // 动态调整卡尔曼参数
...
static void vsn_predict_angle_calc(void)
{
    ...
    /* -------------------------- 逐差与开始预测 -------------------------- */
    if (vd.valid_cnt > stable_time_flt)
    { /* 等待滤波器收敛*/
        /* 视觉相对角速度（°/s） */
...
void vsn_gimbal_ref_calc(void)
{
    ...
    { /* 未识别到目标 */
        ...
        else if (ctrl_mode == AUTO_MODE || ctrl_mode == PROTECT_MODE)
        {
            ...
            if (vision_ctrl.is_detect == 1 && ctrl_mode == AUTO_MODE)
            {

                gimbal.pid.yaw_angle_6020_ref = vision_ctrl.perception_yaw; // 视觉绝对角度
            ...
            else
            {
                ...
                if (protect_mode == 0)
                {
                    ...
                    if (
                        ((ABSv(moto_yaw.ecd - encoder_L) < 10 || ABSv(moto_yaw.ecd - encoder_R) < 100) && (close_flag == 0)))
                    {
                        scan_dir = -scan_dir;
                        close_flag = 1;
                    }
...
static float vsn_gradual_deduction(float array[6])
{
    ...
    return (
        (diff_k1 * error[0] + diff_k2 * error[1] + diff_k3 * error[2]) / (3 * vd.period));
}
...
int rotation_direction_judge(void)
{
    ...
    static int8_t dir;
    if (ABS(vd.yaw.angular_speed.now) > test_threshold)
    { /* 目标有移动 */
        invalid_cnt = 0;
        valid_cnt = 1;
